...
const Products = () => {
  ...
  const filteredProducts = products.filter(product => (selectedCategory === "All" || product.category === selectedCategory) && (product.name?.toLowerCase().includes(searchTerm.toLowerCase()) || product.description?.toLowerCase().includes(searchTerm.toLowerCase()))).sort((a, b) => {
  ...
  return <>
      <div className="bg-gradient-sacred/10 border-b border-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="text-center space-y-4">
            <h1 className="text-4xl font-bold bg-gradient-sacred bg-clip-text text-transparent md:text-7xl my-0 mx-0 px-0 py-[10px]">
              Pooja Products
            </h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {loading ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            {filteredProducts.map(product => <Card key={product.id} className="group transition-all hover:shadow-lg">
                <div className="relative overflow-hidden aspect-square cursor-pointer" onClick={() => navigate(`/products/${product.id}`)}>
                  <img src={product.image_url || poojaProducts} alt={product.name} className="w-full h-full object-cover transition-transform group-hover:scale-110" />
                  <div className="absolute top-3 right-3 flex gap-2">
                    <Button size="icon" variant="secondary" className="rounded-full shadow-lg" onClick={e => {
                e.stopPropagation();
        ...
        {!loading && filteredProducts.length === 0 && <div className="text-center py-12">

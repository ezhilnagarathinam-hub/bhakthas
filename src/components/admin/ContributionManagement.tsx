...
const ContributionManagement = () => {
  ...
  const fetchContributions = async () => {
    const {
      data,
      error
    } = await supabase.from('temple_contributions').select('*').order('created_at', {
      ascending: false
    });
    if (error) {
      toast({
  ...
  const handleStatusChange = async (id: string, status: string) => {
    const {
      error
    } = await supabase.from('temple_contributions').update({
      status
    }).eq('id', id);
    if (error) {
      toast({
    ...
    toast({
      title: "Success",
      description: `Contribution status updated to ${status}`
    });
    fetchContributions();
  };
  ...
  const handleUpdateContribution = async (e: React.FormEvent) => {
    ...
    const {
      error
    } = await supabase.from('temple_contributions').update(updateData).eq('id', selectedContribution.id);
    if (error) {
      toast({
        title: "Error",
        description: error.message,
  ...
  const handleDelete = async () => {
    ...
    const {
      error
    } = await supabase.from('temple_contributions').delete().eq('id', contributionToDelete);
    if (error) {
      toast({
        title: "Error",
        description: error.message,
  ...
  const exportToPDF = () => {
    ...
    autoTable(doc, {
      ...
      body: contributions.map(c => [c.temple_name, c.city, c.state, c.status, new Date(c.created_at).toLocaleDateString()]),
      startY: 20
    });
  ...
  return <>
      <Card className="bg-white border-admin-border shadow-divine">
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="text-red-900">Temple Contributions</CardTitle>
        ...
        <CardContent className="bg-stone-400 text-[#f43713]">
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {contributions.map(contribution => <TableRow key={contribution.id} className="border-admin-border">
                  ...
                  <TableCell>
                    <Select value={contribution.status} onValueChange={value => handleStatusChange(contribution.id, value)}>
                  ...
                  <TableCell>
                    ...
                    setViewDialogOpen(true);
                      ...
                      <Button size="sm" variant="outline" onClick={() => handleEdit(contribution)} className="border-admin-border text-blue-400 hover:bg-admin-bg">
      ...
      <Dialog open={viewDialogOpen} onOpenChange={setViewDialogOpen}>
        <DialogContent className="max-w-2xl bg-admin-surface border-admin-border">
          ...
          {selectedContribution && <div className="space-y-4">
              <div>
              ...
              {selectedContribution.media_url && <div>
                  <h4 className="font-semibold mb-2 text-admin-text">Media</h4>
                  {selectedContribution.media_url.match(/\.(jpeg|jpg|gif|png|webp)$/i) ? <img src={selectedContribution.media_url} alt="Temple" className="w-full rounded-lg" /> : <video src={selectedContribution.media_url} controls className="w-full rounded-lg" />}
                </div>}
            </div>}
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="max-w-2xl bg-admin-surface border-admin-border">
          ...
          <form onSubmit={handleUpdateContribution} className="space-y-4">
            <div>
              ...
              <Input id="temple_name" value={editFormData.temple_name} onChange={e => setEditFormData({
              ...editFormData,
              temple_name: e.target.value
            })} required className="bg-admin-bg border-admin-border text-admin-text" />
            </div>
            <div>
              <Label htmlFor="description" className="text-admin-text">Description</Label>
              <Textarea id="description" value={editFormData.description} onChange={e => setEditFormData({
              ...editFormData,
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input id="city" value={editFormData.city} onChange={e => setEditFormData({
                ...editFormData,
                city: e.target.value
              })} required className="bg-admin-bg border-admin-border text-admin-text" />
              </div>
              <div>
                ...
                <Input id="state" value={editFormData.state} onChange={e => setEditFormData({
                ...editFormData,
            ...
            <div>
              ...
              <Input id="address" value={editFormData.address} onChange={e => setEditFormData({
              ...editFormData,
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input id="latitude" type="number" step="any" value={editFormData.latitude} onChange={e => setEditFormData({
                ...editFormData,
              ...
              <div>
                ...
                <Input id="longitude" type="number" step="any" value={editFormData.longitude} onChange={e => setEditFormData({
                ...editFormData,
